...
#--Run for each block-----------------------------------------------------------
    for(i in 1:blocks$n){
        ...
        referenceValues <- getValues(reference,
                                     row = blocks$row[i],
                                     nrow = blocks$nrow[i]
                                     )
        ...
        if(any(classedValues > 1)){
            stop("Fuzzy memberships cannot be above 1.")
        }
        if(any(classedValues < 0)){
            stop("Fuzzy memberships cannot be below 0.")
        }
...
#--Calculate accuracy percentage------------------------------------------------
    ...
    if(plotOut){
        ...
        plot(c(0, temp),
             c(min(accOut, na.rm = TRUE), max(accOut, na.rm = TRUE)),
             xlab = "Membership",
             ylab = "Accuracy",
             type = "n"
             )
